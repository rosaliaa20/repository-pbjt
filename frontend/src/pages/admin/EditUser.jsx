...
import { FiUser, FiCreditCard, FiMail, FiBookOpen, FiShield, FiSave, FiArrowLeft, FiAlertCircle, FiPhone, FiLock, FiEye, FiEyeOff } from "react-icons/fi";
...
const EditUser = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    department: '',
    role: '',
    auth_password: '' // Sandi konfirmasi untuk mengedit Admin
  });
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
          ...
          {originalRole === 'admin' && (
            <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl">
              <label className="block text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-widest mb-2 ml-1">
                🔒 Otorisasi Sandi Akun (Wajib)
              </label>
              <p className="text-[10px] text-amber-600 dark:text-amber-500 mb-3 ml-1 font-bold">
                Karena target adalah Admin, Anda harus memasukkan sandi asli dari target untuk menyimpan perubahan.
              </p>
              <div className="relative">
                ...
                <input 
                  ...
                  required
                  placeholder="Ketikkan sandi target..."
                  className="w-full pl-11 pr-12 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none text-sm font-medium text-slate-800 dark:text-slate-200 transition-all" 
                />
                <button 
                  type="button"
